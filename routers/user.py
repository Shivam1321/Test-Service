...
def login():
	...
	user_from_db = User.objects(username = login_details['username'])  # search for user in database
	print(repr(user_from_db),"user_from_db")
	if user_from_db:
		encrpted_password = hashlib.sha256(login_details['password'].encode("utf-8")).hexdigest()
		if encrpted_password == user_from_db[0].password:
			access_token = create_access_token(identity=user_from_db[0].username) # create jwt token
