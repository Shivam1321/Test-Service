...
def register():
	...
	if not doc:
		temp = User(username=new_user["username"],email= new_user["email"],password = new_user["password"],user_type = new_user["user_type"])
		temp.save()
...
def login():
	...
	if user_from_db:
		...
		'role': str(user_from_db.user_type)
	}
		if encrpted_password == user_from_db.password:
			access_token = create_access_token(identity=identity) # create jwt token
...
def profile():
	...
	user_from_db = User.objects(pk = current_user['id'])
	if user_from_db:
